...
TMap<EVoxelMaterial, FVoxelDataTableRow*> ChunkMeshTask::DataTableCache;
...
ChunkMeshTask::ChunkMeshTask(AChunk* Owner, const FIntVector& ChunkOffset, const FIntVector& ChunkSize, const int32& VoxelSize, const float& NoiseWeight, const float& NoiseScale, const int32& RandomSeed, UDataTable* VoxelDataTable)
{
	...
	this->VoxelDataTable = VoxelDataTable;
}

ChunkMeshTask::ChunkMeshTask(AChunk* Owner, const FIntVector& ChunkSize, const int32& VoxelSize, const TArray<FVoxel>& VoxelData, const int32& Index, const EVoxelMaterial& VoxelMaterial, UDataTable* VoxelDataTable)
	: Owner(Owner), ChunkSize(ChunkSize), VoxelSize(VoxelSize)
{
	this->VoxelEditIndex = Index;
	this->VoxelEditMaterial = VoxelMaterial;
	this->bIsFirstTime = false;
	this->VoxelDataTable = VoxelDataTable;

	for (auto & Voxel : VoxelData)
	{
		FVoxelFace VoxelFace;
		VoxelFace.Coord = Voxel.Coord;
		SetVoxelFace(VoxelFace, Voxel.Material);
		this->VoxelData.Add(VoxelFace);
	}
}
...
void ChunkMeshTask::DoWork()
{
	...
	{
		SetVoxelFace(VoxelData[VoxelEditIndex], VoxelEditMaterial);
	}
	UpdateMesh();
	...
	{
		FVoxel NewVoxel;
		NewVoxel.Coord = Voxel.Coord;
		NewVoxel.Material = Voxel.Material;
		ResultVoxelData.Add(NewVoxel);
	}

	bIsTaskFinished = true;
}

bool ChunkMeshTask::IsTaskFinished()
{
	return bIsTaskFinished;
}

FVoxelDataTableRow* ChunkMeshTask::GetVoxelDataRow(const EVoxelMaterial& VoxelMaterial)
{
...
void ChunkMeshTask::SetVoxelFace(FVoxelFace& Face, const EVoxelMaterial& Material)
{
	if (Material != EVoxelMaterial::NONE)
	{
		FVoxelDataTableRow* DataTable = GetVoxelDataRow(Material);
		check(DataTable);

		Face.Material = Material;
		Face.HasMesh = DataTable->bIsMesh;
		Face.IsOpacity = DataTable->bIsOpacity;
	}
	else
	{
		Face.Material = EVoxelMaterial::NONE;
		Face.HasMesh = false;
		Face.IsOpacity = false;
	}
}
	...
	for (int32 Z = 0; Z < ChunkSize.Z; Z++)
	{
		for (int32 X = 0; X < ChunkSize.X; X++)
		{
			for (int32 Y = 0; Y < ChunkSize.Y; Y++)
			{
				...
				float Mountain2DMask2 = USimplexNoiseBPLibrary::SimplexNoiseScaledFractalInRange2D(CurrentChunkLocation.X, CurrentChunkLocation.Y, 0.0f, 1.0f, 0.01f, 5, 0.4f, 128.0f, 4.0f, 0.25f);

				float NoiseValue = GroundValue + MountainValue * Mountain2DMask * Mountain2DMask2;
				...
				{
					SetVoxelFace(VoxelFace, EVoxelMaterial::COBBLESTONE);
				}
				else if (NoiseValue + 0 > CurrentChunkLocation.Z)
				{
					SetVoxelFace(VoxelFace, EVoxelMaterial::DIRT);
				}
				else if (NoiseValue + 1 > CurrentChunkLocation.Z)
				{
					SetVoxelFace(VoxelFace, EVoxelMaterial::GRASS);
				}
				else if (NoiseValue + 2 > CurrentChunkLocation.Z && TallGrassStream.FRand() < 0.3f)
				{
					SetVoxelFace(VoxelFace, EVoxelMaterial::TALLGRASS);
					if (!PlantData.Contains(VoxelFace.Material))
					{
						PlantData.Add(VoxelFace.Material, TArray<FIntVector>());
					}

					PlantData[VoxelFace.Material].Add(VoxelFace.Coord);
				}
				else if (NoiseValue + 2 > CurrentChunkLocation.Z && TreeStream.FRand() < 0.1f)
				{
					SetVoxelFace(VoxelFace, EVoxelMaterial::LOG);
	...
	for (const auto & Root : TreeRoots)
	{
		...
		if (IsTreeValid)
		{
			for (int32 Z = 1; Z < 5; Z++)
			{
				...
				SetVoxelFace(VoxelData[Index], EVoxelMaterial::LOG);
			}
			...
			for (int32 X = -1; X <= 1; X++)
			{
				for (int32 Y = -1; Y <= 1; Y++)
				{
					for (int32 Z = 2; Z < 6; Z++)
					{
						...
						{
							SetVoxelFace(VoxelData[Index], EVoxelMaterial::LEAVES);
						}
					}
				}
			}
		...
		else
		{
			...
			SetVoxelFace(VoxelData[Index], EVoxelMaterial::NONE);
		}
	}
